...
function getSearchResults() {
  /**
   * @brief Collate all the input elements into an object and send AJAX request, then render any results
   * @TODO nothing, server-side handles all the validation stuff
   */
  const placementTitle = $("#title-search").val().trim();
  const location = $("#location-search").val();
  const placementType = $("#opportunity-type").val();
  const searchData = {
  ...
  /* AJAX request to the node server */
  $.ajax({
    url: "/search",
    method: "POST",
    data: searchData,
    success: function (response) {
      ...
      response.forEach((opportunity) => {
        ...
        console.table(opportunity.featured);
        console.log(opportunity.featured == true);
        $("#opportunity-list").append(`
                        <li class="homepage-opportunity-listing  
                        ${opportunity.featured == true ? "sponsored" : ""}" oppid="${
          opportunity._id
