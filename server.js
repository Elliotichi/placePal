...
app.post("/login",
  [
    ...
    db.collection("users").findOne({ "email": req.body.email }, (err, result) => {
      ...
      if (crypto.createHash('sha256').update(password).digest('hex') === hashedPassword) {
...
app.post("/analyze", upload.single("document"), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      /* Temporary buffer */
      const textFilePath = path.join(uploadDir, `${path.basename(wordDocPath, path.extname(wordDocPath))}.txt`);
      ...
      const microserviceResult = await axios.post(`http://localhost:${MICROSERVICE_PORT}/process`, newForm, {
        headers: {
          ...newForm.getHeaders()
        }
      });
